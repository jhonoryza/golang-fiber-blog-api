...
<template>
  ...
  >
    ...
    <div
      ...
      v-if="isFilterValuesNotEmpty()"
    >
      <div class="text-sm flex gap-2 items-center">
        <span>Filter</span>
        <div
          class="border border-slate-300 px-1 rounded"
          v-for="key in Object.keys(filter)"
          :key="key"
        >
          <div
            class="flex gap-1 justify-between items-center hover:cursor-pointer hover:text-gray-500"
            @click="removeFilter(key)"
            v-if="filter[key].length > 0"
          >
